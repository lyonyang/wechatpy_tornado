...
from wechatpy_tornado.client import WeChatComponentClient
from wechatpy_tornado.constants import WeChatErrorCode
from wechatpy_tornado.crypto import WeChatCrypto
from wechatpy_tornado.exceptions import APILimitedException, WeChatClientException, WeChatOAuthException, \
    WeChatComponentOAuthException
from wechatpy_tornado.fields import DateTimeField, StringField
from wechatpy_tornado.messages import MessageMetaClass
from wechatpy_tornado.session.memorystorage import MemoryStorage
from wechatpy_tornado.utils import get_querystring, json, to_binary, to_text, ObjectDict
...
class BaseWeChatComponent(object):
    ...
    def __init__(self,
        ...
        if isinstance(session, six.string_types):
            ...
            from wechatpy_tornado.session.shovestorage import ShoveStorage

            querystring = get_querystring(session)
            prefix = querystring.get('prefix', ['wechatpy_tornado'])[0]
    ...
    async def _request(self, method, url_or_endpoint, **kwargs):
        ...
        if isinstance(kwargs['params'], dict) and \
            ...
            params[
                'component_access_token'] = await self.access_token()
...
class WeChatComponent(BaseWeChatComponent):
    ...
    async def get_pre_auth_url(self, redirect_uri):
        ...
        return "{0}?component_appid={1}&pre_auth_code={2}&redirect_uri={3}".format(
            self.PRE_AUTH_URL, self.component_appid, (await self.create_preauthcode())['pre_auth_code'], redirect_uri
        )
    ...
    async def get_pre_auth_url_m(self, redirect_uri):
        ...
        return "{0}component_appid={1}&pre_auth_code={2}&redirect_uri={3}".format(
            url, self.component_appid, (await self.create_preauthcode())['pre_auth_code'], redirect_uri
        )
    ...
    async def set_authorizer_option(
    ...
    async def get_client_by_authorization_code(self, authorization_code):
        """
        通过授权码直接获取 Client 对象
        ...
        result = await self.query_auth(authorization_code)
        access_token = result['authorization_info']['authorizer_access_token']
    ...
    async def get_client_by_appid(self, authorizer_appid):
        ...
        if not access_token:
            ret = await self.refresh_authorizer_token(
                authorizer_appid,
                refresh_token
            )
