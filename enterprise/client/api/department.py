...
class WeChatDepartment(BaseWeChatAPI):
    ...
    async def create(self, name, parent_id=1, order=None, id=None):
        """
        创建部门
    ...
    async def update(self, id, name=None, parent_id=None, order=None):
        """
        更新部门
    ...
    async def get_users(self, id, status=0, fetch_child=0, simple=True):
        """
    ...
    async def get_map_users(self, id=None, key='name', status=0, fetch_child=0):
        """
        映射员工某详细字段到 ``user_id``
        ...
        """
        ids = [id] if id is not None else [item['id'] for item in await self.get()]
        users_info = list(chain(*[
            await self.get_users(department, status=status, fetch_child=fetch_child, simple=False)
